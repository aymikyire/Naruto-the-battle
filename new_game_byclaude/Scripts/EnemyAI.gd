...
func _ready():
	...
	target = null
	camp_target = null
	_patrol_index = 2  # 从最近的右营地(1300,600)开始
	_patrol_target_pos = CAMP_POSITIONS[2]
	# 同时尝试查找真实野怪
	find_farming_target()
...
func make_decision():
	...
	if skill_count < 2:
		...
		if state != State.FARMING:
			state = State.FARMING
			has_monsters = find_farming_target()
		elif not _is_farming_target_valid():
			has_monsters = find_farming_target()
		else:
			has_monsters = true
		# 全地图没活怪  →  暂时去找玩家
		if not has_monsters and player and is_instance_valid(player):
			target = player
			state = State.SEEKING
		return
...
# 搜索最近的活野怪作为目标，返回true=找到活怪，false=全地图没活怪
func find_farming_target() -> bool:
	# 1. 优先找活的野怪（全地图）
